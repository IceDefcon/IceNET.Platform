...
static int __init i2c_module_init(void)
{
    ...
    /* L3G42000 */
    int ret = i2c_smbus_read_i2c_block_data(i2c_client, 0x0F, sizeof(read_buffer), read_buffer);
    if (ret < 0) {
    ...
    // char write_buffer[2] = {0x01, 0x02};
    // ret = i2c_smbus_write_i2c_block_data(i2c_client, 0x00, sizeof(write_buffer), write_buffer);
    // if (ret < 0) {
