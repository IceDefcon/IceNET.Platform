...
tcpServer::~tcpServer() 
{
    std::cout << "[NET] Destroying tcpServer" << std::endl;
    closeClient();
    if (serverSocket >= 0) 
    {
        close(serverSocket);
...
bool tcpServer::initialize() 
{
    ...
    if (serverSocket < 0) 
    {
        perror("[NET] Socket Server: error opening socket");
        return false;
    }
    ...
    {
        perror("[NET] Socket Server: error setting socket options");
        return false;
    }

    if (bind(serverSocket, (struct sockaddr *)&serverAddress, sizeof(serverAddress)) < 0) 
    {
        perror("[NET] Socket Server: error on binding the socket");
        return false;
    }
    ...
    {
        perror("[NET] Socket Server: error listening for connections");
        return false;
    }
...
bool tcpServer::acceptClient() 
{
    ...
    if (clientSocket < 0) 
    {
        perror("[NET] Socket Server: failed to accept the client connection");
        return false;
    }
...
ssize_t tcpServer::dataTX(const std::string& message) 
{
    if (!clientConnected) 
    {
        std::cerr << "[NET] Socket Server: no client connected" << std::endl;
        return -1;
    }
...
std::string tcpServer::dataRX(size_t bufferSize) 
{
    ...
    if (bytesRead < 0) 
    {
        perror("[NET] Socket Server: error reading from socket");
        return "";
    }
